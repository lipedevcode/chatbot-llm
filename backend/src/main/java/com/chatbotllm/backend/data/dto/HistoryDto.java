...
public record HistoryDto (Long id, List<PromptDto> prompts){

    public static HistoryDto fromHistory(Long id, List<Prompt> prompts) {
        List<PromptDto> promptsDto = prompts.stream()
                ...
                .toList();
        return new HistoryDto(id, promptsDto);
    }
}
